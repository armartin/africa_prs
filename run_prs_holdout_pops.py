...
def main(args):
    ...
    if args.mt_comb:
        ...
        mt_all = mt_all.filter_cols(hl.or_else(ht_samples[mt_all.s].in_gwas != 'TRUE', True))
        # print(mt.count()) # 13364303, 136265)
        mt_all.write('gs://apcdr/ukb_holdout/ukb31063.gwas_holdout_sumstats_pheno37_subset.mt', overwrite=args.overwrite)

    mt_all = hl.read_matrix_table('gs://apcdr/ukb_holdout/ukb31063.gwas_holdout_sumstats_pheno37_subset.mt')
    ...
    for pheno in apcdr_phenos.keys():
        ...
        annot_expr = {
            k: hl.agg.sum(mt.ss.beta[phenotypes.index(apcdr_phenos[pheno])][0] * mt.dosage * hl.int(
                mt.ss.p_value[phenotypes.index(apcdr_phenos[pheno])][0] < v))
    ...
    #          's10': 1}
    #
    # print('Read in clumps: ' + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    # pheno_clump = {}
    # for pheno in apcdr_phenos.keys():
    #     pheno_clump[pheno] = specific_clumps('gs://apcdr/ukb_holdout/ukb31063.gwas_holdout.' + pheno + '_v3.clumped')
    #
    # mt = mt.annotate_globals(clump_data=pheno_clump)
    #
    # annot_expr = hl.struct(**{pheno: hl.struct(**{
    #     k: hl.agg.filter(mt.clump_data[pheno].get(mt.locus, False), hl.agg.sum(
    #         mt.ss.beta[phenotypes.index(apcdr_phenos[pheno])][0] * mt.dosage * hl.int(
    #             mt.ss.p_value[phenotypes.index(apcdr_phenos[pheno])][0] < v)))
    #     for k, v in p_max.items()})
    # for pheno in list(apcdr_phenos.keys())})
    #
    # mt = mt.annotate_cols(pheno_prs=annot_expr)
    # ht_out = mt.cols()
    # ht_out = ht_out.checkpoint('gs://apcdr/ukb_holdout/ukb31063.gwas_holdout.PRS.ht')
    # ht_out = ht_out.flatten().flatten()
    # ht_out.export('gs://apcdr/ukb_holdout/ukb31063.gwas_holdout.PRS.txt.bgz')
    #
    # end = time.time()
    # print("Success! Job was completed in %s" % time.strftime("%H:%M:%S", time.gmtime(end - start)))
